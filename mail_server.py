...
def add_mail(mail_entry: Dict[str, str]) -> str:
    """
    Appends a mail entry onto the mail in the json file, generates a unique id for the entry, and saves it
...
def delete_mail(mail_id: str) -> bool:
    """
    Deletes a mail entry from the mail in the json file
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    """
    Gets the list of mail received by a particular recipient
...
def get_sent(sender: str) -> List[Dict[str, str]]:
    """
    Gets the list of mail sent by a particular sender
...
def delete_mail_route(mail_id: str):
    ...
    success = delete_mail(mail_id)
    
    res = jsonify({'return:' : True})
